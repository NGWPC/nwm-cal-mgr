...
import pandas as pd
import yaml
from pydantic import BaseModel, DirectoryPath, Field, conint, validator
...
class EvaluationOptions(BaseModel):
    ...
    _objective_log_file: Path = None
    _metric_iter_file: Path = None
    _param_iter_file: Path = None
    _param_all_file: Path = None
    _output_iter_file: Path = None
    _last_output_file: Path = None
    _best_output_file: Path = None
    _last_iter_file: Path = None
    _cost_iter_file: Path = None
    ...
    def __init__(self, **kwargs):
        ...
        self._last_output_file = kwargs.pop(
            "last_output_file", Path("{}_output_last_iteration.csv".format(self.basinID))
        )
        self._best_output_file = kwargs.pop(
            "best_output_file", Path("{}_output_best_iteration.csv".format(self.basinID))
        )
    ...
    def write_objective_log_file(self, i: int, score: float) -> None:
        ...
        if not os.path.exists(self._objective_log_file):
            with open(self._objective_log_file, "w") as log_file:
                log_file.writelines(
                    ["iteration,", "objective_function,", "best_iteration,", "best_objective_function\n"]
                )
    ...
    def write_metric_iter_file(self, i: int, score: float, metrics: float) -> None:
        ...
        metric_current.to_csv(
            self._metric_iter_file, mode="a", index=False, header=not os.path.exists(self._metric_iter_file)
        )
    ...
    def write_param_iter_file(self, i: int, params: "DataFrame") -> None:
        ...
        param_current.to_csv(
            self._param_iter_file, mode="a", index=False, header=not os.path.exists(self._param_iter_file)
        )
        ...
        for n in range(0, i + 1):
            ...
            if df_log_iter.shape[0] > 0:
                best_cost = pd.DataFrame(
                    {"iteration": n, "global_best": df_log_iter["best_objective_function"].min()}, index=[0]
                )
    ...
    def create_valid_config_file(
        self, yaml_file: Path, valid_run_path: Path, valid_config_file: Path, valid_run_name: str
    ) -> None:
    ...
    def write_run_complete_file(self, run_name: str, path: "Path") -> None:
        ...
        complete_file = os.path.join(
            path, "{}".format(self.basinID) + "_" + "{}".format(run_name).capitalize() + "_Run_Complete"
        )
    ...
    def objective_log_file(self) -> Path:
        ...
        if id is not None:
            ...
            prefix = f"{self.id}_"
        return Path(
            self._objective_log_file.parent, prefix + self._objective_log_file.stem + self._objective_log_file.suffix
        )
    ...
    def restart(self) -> int:
        ...
        shutil.copy(
            str(self._last_iter_file), str(self._last_iter_file) + "_before_restart_" + time.strftime("%Y%m%d_%H%M%S")
        )

...
from pydantic import ConfigDict, Field, FilePath, RootModel, model_validator
...
class NgenBase(ModelExec):
    ...
    def set_defaults(cls, values: Dict):
        ...
        if args is None:
            ...
            args = '{} "all" {} "all" {}'.format(
                Path(catchments).resolve(), Path(nexus).resolve(), Path(realization).name
            )
            values["args"] = args
        else:
            custom_args = True
    ...
    @model_validator(mode="before")  # pre-check, don't validate anything else if this fails
        ...
        for id, catchment in self.ngen_realization.catchments.items():
            if hasattr(catchment, "calibration"):
                ...
                try:
                    ...
                    raise (
                        RuntimeError(
                            "Cannot establish mapping of catchment {} to nwis location in cross walk".format(id)
                        )
                    )
                try:
                ...
                self._catchments.append(
                    CalibrationCatchment(
                        self.workdir, id, nexus, start_t, end_t, fabric, output_var, eval_params, params
                    )
                )
...
class NgenIndependent(NgenBase):
    ...
    def __init__(self, **kwargs):
        ...
        self._catchments.append(
            CalibrationSet(catchments, eval_nexus[0], self.routing_output, start_t, end_t, self.eval_params)
        )
...
class Ngen(RootModel[Union[NgenExplicit, NgenIndependent, NgenUniform]], Configurable):
    model_config = ConfigDict(smart_union=True)

    # proxy methods for Configurable
    def get_args(self) -> str:
        return self.root.get_args()

    def get_binary(self) -> str:
        return self.root.get_binary()

    def update_config(self, *args, **kwargs):
        return self.root.update_config(*args, **kwargs)

    # proxy methods for model
    @property
    def adjustables(self):
        return self.root._catchments
    ...
    def resolve_paths(self):
        ...
        if self.root.ngen_realization != None:
            self.root.ngen_realization.resolve_paths()

    @property
    def best_params(self):
        return self.root.eval_params.best_params

    @property
    def df_precip(self):
        return self.root._precip
